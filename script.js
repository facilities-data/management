...
async function loadAllData(keys = Object.keys(TABLES)) {
    await Promise.all(keys.map(key => loadTable(key)));
}
...
async function startPresence(user) {
    if (!user || presenceChannel || !supabaseClient?.channel) {
        return;
    }
    ...
    // Always show this browser session, even if Realtime is unavailable.
    renderActiveUsers([localUser]);
    clearTimeout(presenceRetryTimer);

    const channel = supabaseClient.channel("fms-active-users", {
    ...
    const updateUsers = () => {
        ...
        renderActiveUsers(users.length ? users : [localUser]);
    };
    ...
    channel.subscribe(async status => {
        if (status === "SUBSCRIBED") {
            const { error } = await channel.track(localUser);

            if (error) {
                console.warn("Unable to publish active-user presence:", error.message);
                renderActiveUsers([localUser]);
                return;
            }
        ...
        if (["CHANNEL_ERROR", "TIMED_OUT", "CLOSED"].includes(status)) {
            // A channel error should not make the local user disappear.
            console.warn(`Active-user presence unavailable: ${status}`);
            renderActiveUsers([localUser]);
            ...
            // Retry later in case the connection was temporarily unavailable.
            presenceRetryTimer = setTimeout(() => {
                getCurrentUser().then(startPresence).catch(() => {});
            }, 30000);
        }
    });
}
...
function renderCalendar() {
    ...
    for (let index = 0; index < 52; index++) {
        ...
        const tasks = cache.pms
            ...
            .map(task => `
                <div class="maintenance-dot ${task.status === "Done" ? "done" : task.status === "In Progress" ? "in-progress" : ""}">
...
async function renderAll(loadData = true) {
