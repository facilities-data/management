...
function getStatusBadge(status) {
    ...
    if (
        ...
        className = "progress";
    } else if (status === "Out of Service" || status === "Failed") {
        className = "cancelled";
    }
...
function createSafetyEquipmentId() {
    return createId("SE-", cache.safety, 5, 0);
}
...
function updateNavigationNotifications() {
    ...
    const counts = {
        ...
        "safety-equipment-count": cache.safety.filter(item =>
            item.status !== "Passed"
        ).length,
...
function normalizeInspectionMonth(month) {
    if (!month) {
        return "";
    }

    // Supabase date columns require a complete YYYY-MM-DD value.
    return /^\d{4}-\d{2}$/.test(month)
        ? `${month}-01`
        : month;
}

function getInspectionMonthValue(month) {
    return String(month || "").slice(0, 7);
}

function getInspectionMonthLabel(month) {
    const monthValue = getInspectionMonthValue(month);
...
function getInspectionDateForMonth(month, existingDate = "") {
    const monthValue = getInspectionMonthValue(month);
    const dateValue = String(existingDate || "").slice(0, 10);

    if (!monthValue) {
        return "";
    }

    // Preserve a saved inspection date only when it belongs to this month.
    if (dateValue.startsWith(`${monthValue}-`)) {
        return dateValue;
    }

    // A newly selected month must never inherit today's date. Use the first
    // day of the selected month as the initial inspection date.
    return `${monthValue}-01`;
}

function getSafetyInspectionHistory(item) {
    let history = item?.inspection_history;
    ...
    // Use browser storage as a fallback when the optional database column is
    // not available. This also keeps the calendar history visible immediately.
    if (item?.id) {
        try {
            const localHistory = JSON.parse(
                localStorage.getItem(`fms-safety-history-${item.id}`) || "{}"
            );
            history = { ...localHistory, ...history };
        } catch (error) {
            console.warn("Unable to read local inspection history:", error);
        }
    }
...
function getLatestSafetyInspection(item) {
    const history = getSafetyInspectionHistory(item);
    const entries = Object.entries(history)
        .map(([month, activity]) => ({
            month,
            inspected: String(activity?.inspected || "").slice(0, 10),
            status: activity?.status || "",
            remarks: activity?.remarks || ""
        }))
        .filter(entry => entry.inspected)
    ...
    return {
        month: getInspectionMonthValue(item?.inspection_month),
        inspected: String(item?.inspected || "").slice(0, 10),
        status: item?.status || "",
        remarks: item?.remarks || ""
    };
}
...
function getSafetyMonthCalendar(item) {
    ...
    const months = Array.from({ length: 12 }, (_, index) => {
        ...
        return `
            <button type="button"
                class="inspection-month ${date ? "inspected" : ""}"
                data-safety-month="${escapeHtml(item.id)}"
                data-month="${month}"
                title="${date ? `Inspected ${date}${status ? ` · ${status}` : ""}` : "Record inspection for this month"}">
                <span>${new Date(`${month}-01T00:00:00`).toLocaleDateString("en-US", { month: "short" })}</span>
                <small>${date ? escapeHtml(date) : "—"}</small>
            </button>
        `;
    }).join("");
...
function openSafetyInspection(id, month) {
    const item = cache.safety.find(record => record.id === id);

    if (!item) {
        return;
    }

    editSafetyEquipment(id, month);
}

function renderSafetyEquipment() {
    const body = document.querySelector("#safety-equipment-table tbody");
    const search = (getElement("safety-equipment-search")?.value || "").toLowerCase();
    ...
    cache.safety
        .filter(item => Object.values(item).join(" ").toLowerCase().includes(search))
        .forEach(item => {
            const latestInspection = getLatestSafetyInspection(item);
            const row = document.createElement("tr");
            row.innerHTML = `
                ...
                <td>${escapeHtml(getInspectionMonthLabel(latestInspection.month))}</td>
                <td>${escapeHtml(latestInspection.inspected)}</td>
                <td>${getStatusBadge(latestInspection.status)}</td>
                <td>${escapeHtml(latestInspection.remarks)}</td>
                <td>${getSafetyMonthCalendar(item)}</td>
                <td>
                    <button class="btn-action btn-primary" data-edit-safety="${escapeHtml(item.id)}">Edit</button>
                    <button class="btn-action btn-danger" data-delete-safety="${escapeHtml(item.id)}">Delete</button>
                </td>
            `;
            body.appendChild(row);
        });

    body.querySelectorAll("[data-edit-safety]").forEach(button => {
        button.onclick = () => editSafetyEquipment(button.dataset.editSafety);
    });

    body.querySelectorAll("[data-safety-month]").forEach(button => {
        button.onclick = () => openSafetyInspection(
            button.dataset.safetyMonth,
            button.dataset.month
        );
    });

    body.querySelectorAll("[data-delete-safety]").forEach(button => {
        button.onclick = async () => {
            const authorized = await requireAdminPassword("delete this safety equipment record");
            if (authorized && confirm("Delete this safety equipment record?")) {
                await deleteRecord("safety", button.dataset.deleteSafety);
                await renderAll();
            }
        };
    });
...
function updateInspectionDateForSelectedMonth(preserveExistingDate = true) {
    const monthInput = getElement("safety-equipment-month");
    const inspectedInput = getElement("safety-equipment-inspected");
    const month = getInspectionMonthValue(monthInput?.value);

    if (!monthInput || !inspectedInput) {
        return;
    }
    ...
    inspectedInput.min = firstDate;
    inspectedInput.max = lastDate;

    // Keep a saved date only when it belongs to the month currently shown.
    // Otherwise, start the modal on the first date of the selected month.
    inspectedInput.value = getInspectionDateForMonth(
        month,
        preserveExistingDate ? inspectedInput.value : ""
    );
}

function editSafetyEquipment(id = "", selectedMonth = "") {
    const item = cache.safety.find(record => record.id === id);
    const savedMonth = getInspectionMonthValue(item?.inspection_month);
    const month = getInspectionMonthValue(selectedMonth || savedMonth);
    const history = getSafetyInspectionHistory(item);
    const activity = history[month] || {};

    getElement("safety-equipment-id").value = item?.id || "";
    getElement("safety-equipment-tag").value = item?.id || createSafetyEquipmentId();
    getElement("safety-equipment-type").value = item?.type || "";
    getElement("safety-equipment-name").value = item?.name || "";
    getElement("safety-equipment-location").value = item?.location || "";
    getElement("safety-equipment-month").value = month;
    getElement("safety-equipment-inspected").value = activity.inspected || "";

    // Set the value, min, and max before opening the modal so the native date
    // picker opens on the selected inspection month, not on the current month.
    updateInspectionDateForSelectedMonth(true);

    getElement("safety-equipment-status").value = activity.status || "";
    getElement("safety-equipment-remarks").value = activity.remarks || "";
    openModal("safety-equipment-modal");
}

async function saveSafetyEquipment(event) {
    event.preventDefault();
    const originalId = getElement("safety-equipment-id").value;
    const enteredId = getElement("safety-equipment-tag").value.trim();
    const id = enteredId || originalId || createSafetyEquipmentId();
    const normalizedId = id.toLowerCase();

    // Check the local cache first for a quick response, ignoring letter case.
    const duplicateInCache = cache.safety.some(item =>
        String(item.id || "").trim().toLowerCase() === normalizedId &&
        item.id !== originalId
    );

    if (duplicateInCache) {
        showNotification(`Barcode ID “${id}” already exists. Enter a unique barcode ID.`);
        getElement("safety-equipment-tag")?.focus();
        return;
    }

    // Query Supabase as well so records created by another user are detected.
    if (!originalId || id.toLowerCase() !== originalId.toLowerCase()) {
        const { data: existing, error: duplicateCheckError } = await supabaseClient
            .from(TABLES.safety)
            .select("id")
            .ilike("id", id)
            .limit(1);
        ...
        if (duplicateCheckError) {
            console.error("Unable to check safety equipment barcode ID:", duplicateCheckError);
            showNotification("Unable to verify the barcode ID. Please try again.");
            return;
        }
    ...
    const inspectionMonth = getInspectionMonthValue(
        getElement("safety-equipment-month").value
    );
    const item = {
        id,
        type: getElement("safety-equipment-type").value,
        name: getElement("safety-equipment-name").value.trim(),
        location: getElement("safety-equipment-location").value.trim(),
        inspection_month: normalizeInspectionMonth(inspectionMonth),
        inspected: getElement("safety-equipment-inspected").value,
        status: getElement("safety-equipment-status").value,
        remarks: getElement("safety-equipment-remarks").value.trim()
    };

    // Save the core safety-equipment fields first. This keeps saving compatible
    // with databases that do not have the optional inspection_history column.
    const existingItem = cache.safety.find(record => record.id === originalId);
    const history = getSafetyInspectionHistory(existingItem);
    history[inspectionMonth] = {
        inspected: item.inspected,
        status: item.status,
        remarks: item.remarks
    };
    ...
    if (saved) {
        ...
        const { error: historyError } = await supabaseClient
            .from(TABLES.safety)
            .update({ inspection_history: history })
            .eq("id", id);

        // A missing optional column is expected on older schemas; do not show
        // it as a save error because the core record was already saved.
        if (historyError && !historyError.message?.includes("inspection_history")) {
...
function getSafetyReportRows() {
    const monthNames = [
        "Jan", "Feb", "Mar", "Apr", "May", "Jun",
        "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
    ];

    return cache.safety.map(item => {
        const history = getSafetyInspectionHistory(item);
        const activities = Object.entries(history)
            ...
            .filter(activity => activity.inspected);

        const legacyMonth = getInspectionMonthValue(item.inspection_month);
        if (item.inspected && !activities.some(activity =>
            activity.month === legacyMonth && activity.inspected === String(item.inspected).slice(0, 10)
        )) {
            activities.push({
                month: legacyMonth,
                inspected: String(item.inspected).slice(0, 10),
        ...
        monthNames.forEach((monthName, index) => {
            ...
            const monthActivities = activities
                .filter(activity => activity.month.slice(5, 7) === monthNumber)
                .sort((a, b) => a.inspected.localeCompare(b.inspected));
            const activity = monthActivities[monthActivities.length - 1] || {};

            row[monthName] = formatReportDate(activity.inspected);
        });

        const latest = activities.sort((a, b) =>
            a.inspected.localeCompare(b.inspected)
        )[activities.length - 1] || {};

        row["Current Status"] = latest.status || item.status || "";
        row.Remarks = latest.remarks || item.remarks || "";
        return row;
    });
}
...
function downloadReport(title, rows, fileName) {
    ...
    const isSafetyReport = fileName === "safety-equipment";

    const header = isSafetyReport
        ? `<tr>
            <th colspan="4"></th>
            <th colspan="12">Inspection Month</th>
            <th colspan="2"></th>
        </tr>
...
async function scanBarcodeFrame() {
    ...
    try {
        ...
        if (barcode) {
            ...
            const records = cache[barcodeScanTarget] || [];
            ...
            if (!record) {
                showNotification(`Barcode “${scannedTag}” was not found in the ${barcodeScanTarget === "safety" ? "safety equipment register" : "asset register"}.`);
                return;
            }

            if (barcodeScanTarget === "safety") {
                editSafetyEquipment(record.id);
            } else {
                editAsset(record.id);
            }
            showNotification(`Barcode “${scannedTag}” found.`);
            return;
        }
    } catch (error) {
...
async function startBarcodeScanner(resetTarget = true) {
    if (resetTarget) {
        barcodeScanTarget = "assets";
    }
...
function startSafetyBarcodeScanner() {
    barcodeScanTarget = "safety";
...
function setupEventHandlers() {
    ...
    getElement("safety-equipment-form")?.addEventListener("submit", saveSafetyEquipment);
    ...
    getElement("scan-safety-barcode")?.addEventListener("click", startSafetyBarcodeScanner);
    ...
    getElement("add-safety-equipment")?.addEventListener("click", () => editSafetyEquipment());
    getElement("safety-equipment-month")?.addEventListener(
        "change",
        () => updateInspectionDateForSelectedMonth(false)
    );
    getElement("safety-equipment-search")?.addEventListener("input", renderSafetyEquipment);
    ...
    const reports = [
        ...
        ["download-safety-report", "Safety Equipment", "safety-report", "safety-equipment"],
    ...
    reports.forEach(([buttonId, title, key, fileName]) => {
        getElement(buttonId)?.addEventListener(
            ...
            () => downloadReport(
                title,
                key === "safety-report" ? getSafetyReportRows() : cache[key],
                fileName
            )
        );
    });
